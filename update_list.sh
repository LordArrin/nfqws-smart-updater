...
LOCK_DIR="/tmp/lock/nfqws_updater.lockdir"
MIN_LINES=10
...
setup_environment() {
    ...
    # Исправлено: используется глобальная переменная LOCK_DIR вместо локальной lock_dir
...
acquire_lock() {
    ...
    exec 200>"$LOCK_FILE"
    # Пытаемся захватить эксклюзивную блокировку без ожидания (-n)
    if ! flock -n 200; then
        log_warn "Скрипт уже выполняется. Выходим."
        exit 0
    fi
    # Блокировка автоматически снимется при завершении скрипта
}

cleanup() {
    if [ -n "${TMPDIR:-}" ] && [ -d "${TMPDIR}" ]; then
        # Проверяем, что путь действительно начинается с TMP_BASE
        if [[ "$TMPDIR" == "$TMP_BASE/"* ]]; then
            rm -rf "$TMPDIR"
        else
            log_err "Аномальный TMPDIR ($TMPDIR), очистка отменена!"
        fi
    fi
}
...
check_dependencies() {
    # Добавлена проверка утилиты timeout
    for cmd in curl sipcalc flock md5sum awk sort timeout; do
...
send_mqtt_notification() {
    ...
    mosquitto_pub -h "$MQTT_HOST" -p "$MQTT_PORT" -u "$MQTT_USER" -P "$MQTT_PASS" -r \
        -t "$MQTT_TOPIC_STATE" -m "$payload" || log_warn "Failed to send MQTT state"
}
